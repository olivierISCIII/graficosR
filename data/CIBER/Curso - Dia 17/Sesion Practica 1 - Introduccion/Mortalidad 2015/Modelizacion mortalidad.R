tee.2015 <- read.csv2("tee.2015.csv",encoding="UTF-8")

edad <- c(4:15)
age <- (edad*5)-2.5
a <- rep(0,nrow(tee.2015))
k <- rep(0,nrow(tee.2015))
ca <- 0.005
ck <- 1
for (i in 1:nrow(tee.2015))
	{
	datos.tee <- tee.2015[i,6:17]
	tee<- as.numeric((datos.tee/100000))
	for(j in 1:12)
		{
		if(tee[j]<=0){tee[j] <- 0.000001}
		}
...
tee.2015 <- cbind(tee.2015[,1:2],a,k)

write.csv2(tee.2015,"mortalidad.2015.csv", row.names=FALSE, fileEncoding="UTF-8")
